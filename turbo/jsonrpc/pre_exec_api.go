...
func (api *PreExecAPIImpl) TraceMany(ctx context.Context, origins []PreArgs) ([]PreResult, error) {
	...
	block, err := api.BaseAPI.blockWithSenders(ctx, tx, hash, blockNumber)
	if err != nil {
		return nil, err
	}
	if block == nil {
	...
	for i := 0; i < len(origins); i++ {
		...
		state.Prepare(rules, msg.From(), blockCtx.Coinbase, msg.To(), vm.ActivePrecompiles(rules), msg.AccessList())
